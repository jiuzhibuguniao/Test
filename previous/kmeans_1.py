#################################
# k-means: 逐个读取文件夹下的文件k-means cluster
# Author: Rooobins
# Data: 2018-11-28
#################################
...
def main():
    path_1="/root/Data_total/"
    path_2="/root/Data_canopy/"
    for file in os.listdir(path_1):
        centroids = loadDataSet(path_2+file[0:19]+"_canopy.txt")
        dataSet_1 = loadDataSet(path_1+file)
        k = len(loadDataSet(path_2+file[0:19]+"_canopy.txt"))
        ...
        with open(path_2+file[0:19]+"_kmeans_centroids.txt", "w+") as f_1:
            f_1.writelines(str(centroids))
            f_1.close()
        with open(path_2+file[0:19]+"_kmeans_clusterAssment.txt", "w+") as f_2:
...
if __name__ == '__main__':
    ...
    t_e = datetime.now()
    usedtime = t_e - t_s
    with open("/root/time.txt",'w+',encoding="UTF-8") as t:
