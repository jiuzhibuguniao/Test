...
proxy_pool=[]
with open("/root/proxy.txt",'r',encoding="UTF-8") as f:
    f_read=f.readlines()
...
class Crawler:
    ...
    def request(self, headers, payload, args, url):
        while True:
            ...
            proxyes={'http':proxy_pool[np.random.randint(0,len(proxy_pool))],'https':proxy_pool[np.random.randint(0,len(proxy_pool))]}
            try:
                response = requests.request(
                    "POST", url, data=payload, headers=headers,
                    proxies={"https": proxyes},
